...
- `onInputChange(field: keyof T, value: string): void` - обрабатывает изменения в полях формы и вызывает событие изменения значения.
...
- `paymentByCash: HTMLButtonElement` - элемент кнопки выбора оплаты при получении (наличными);
- `paymentByCard: HTMLButtonElement` - элемент кнопки выбора оплаты онлайн (картой);
...
Конструктор класса принимает на вход элемент контейнера, а также коллбэк для кнопки завершения заказа.
